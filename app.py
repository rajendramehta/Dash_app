# Flask web application for text replacement in PDF, CSV, XML, XPT files and ZIP archives
...
# Create uploads directory (absolute path for Azure compatibility)
...
# Create a Dash instance, passing the Flask server to it
dash_app = dash.Dash(__name__, server=app, url_base_pathname='/dashboard/')

# Example Dash layout with a simple graph
dash_app.layout = html.Div(children=[
    html.H1(children='Interactive Data Dashboard', style={'textAlign': 'center'}),

    html.Div(children='A simple Dash dashboard embedded in a Flask app.', style={'textAlign': 'center'}),
...
@app.route('/dashboard')
def dashboard():
    """Serve the Dash dashboard page"""
    return render_template('dashboard.html')


@app.route('/upload', methods=['POST'])
def upload_file():
    """Main endpoint: process uploaded files and return modified versions"""
    try:
        # Get form data
        ...
        # Return single file or ZIP of multiple files
        ...
        # Schedule file cleanup after 15 seconds (allows download to complete)
        def remove_files():
            try:
